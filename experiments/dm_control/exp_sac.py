...
import wandb
from average_reward_drl import SAC, eval_actor_dm_control, fix_seed
...
def main():
    ...
    for step in range(args.total_steps):
        ...
        if next_timestep.last():
            timestep = env.reset()
            wandb.log({"step": step, "train/returns": reward_sum})
            print(
        ...
        if step % 5000 == 0 and agent.just_updated:
            ...
            wandb.log({"step": step, "eval/returns": returns.mean()})
            print(
                colorama.Fore.RED
